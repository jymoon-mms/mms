...
def analyze(data: np.ndarray, n_segments: int, peak: float, roi: float):
    ...
    for i in range(n_segments):
        ...
        seg_mean = float(np.mean(seg_data)) if len(seg_data) > 0 else np.nan
        segment_means.append(seg_mean)
        segment_info.append((s_start, s_end, len(seg_data), seg_mean))
...
# ── check.dat 출력 (10개 구간 검증용) ────────────────────────────────────────
def write_check(data: np.ndarray, segment_info: list, peak: float, roi: float,
    ...
    with open(outfile, "w") as f:
        ...
        for i, (s_start, s_end, count, seg_mean) in enumerate(segment_info):
            if i < n_segments - 1:
                mask = (data_roi["sweeps"] >= s_start) & (data_roi["sweeps"] < s_end)
            else:
                mask = (data_roi["sweeps"] >= s_start) & (data_roi["sweeps"] <= s_end)
...
def main():
    ...
    for n in segment_counts:
        ...
        valid = sum(1 for _, _, c, _ in seg_info if c > 0)
    ...
    print()
    if result_10:
        write_check(data, result_10, args.peak, args.roi)

    # 10개 구간 상세 출력
    print()
    print("10개 구간 상세:")
    print(f"{'구간':>5}  {'sweep 시작':>12}  {'sweep 끝':>12}  {'데이터 수':>10}  {'구간 평균':>18}")
    print("-" * 65)
    for i, (s_start, s_end, count, seg_mean) in enumerate(result_10):
        mean_str = f"{seg_mean:>18.4f}" if not np.isnan(seg_mean) else f"{'데이터 없음':>18}"
        print(f"{i+1:>5}  {s_start:>12.1f}  {s_end:>12.1f}  {count:>10}  {mean_str}")
